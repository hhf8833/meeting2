 ...
 * 2.都为空，则返回null
 * 3.有一个为空 左为空，则右不空，pq不可能在左，所以返回右，同时具体又细分为，
 *                      1）右的两个子节点都不空,返回它最近的公共节点即root，而另一边递归肯定找不到pq，所以会返回空，接着在进行判断时返回不空的一边就行了
 *                      2）有一个空，一个不空，返回不空的那个，在某一次递归的另一边会发现另一个不空，所以将root返回
 * 4.同理 有一个为空 右为空，则左不空
 */
