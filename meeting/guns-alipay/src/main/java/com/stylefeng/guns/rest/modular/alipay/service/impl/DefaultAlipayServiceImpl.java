...
@Slf4j
@Component
@Service(interfaceClass = AliPayServicerAPI.class,filter = "tracing")
public class DefaultAlipayServiceImpl implements AliPayServicerAPI {
    ...
    public AliPayResultVO getOrderStatus(String orderId) {
        ...
        //隐式获得user身份
        String userIds = RpcContext.getContext().getAttachment("userId");
    ...
    public String trade_precreate(String orderId) {
        ...
        String sellerId = "2088621958668568";

        // 订单描述，可以对交易或商品进行一个详细地描述，比如填写"购买商品2件共15.00元"
        String body = "购买电影票共花费"+totalAmount;

        // 商户操作员编号，添加此参数可以为商户操作员做销售统计
        String operatorId = "test_operator_id";

        // (必填) 商户门店编号，通过门店号和商家后台可以配置精准到门店的折扣信息，详询支付宝技术支持
        String storeId = "yingyuangongsi";

        // 业务扩展参数，目前可添加由支付宝分配的系统商编号(通过setSysServiceProviderId方法)，详情请咨询支付宝技术支持
        ExtendParams extendParams = new ExtendParams();
        extendParams.setSysServiceProviderId("2088621958668568");
  ...
  /*       // 创建一个商品信息，参数含义分别为商品id（使用国标）、名称、单价（单位为分）、数量，如果需要添加商品类别，详见GoodsDetail
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                filePath= String.format("C:/Users/HP/Desktop/新建文件夹/qr-%s.png",
